<template>
    <v-dialog v-model="show" max-width="400px">
        <v-card>
            ...
            <v-card-text>
                <p>验证码将发送到邮箱 {{ email }}</p>
                <v-text-field v-model="verifyCode" label="验证码" :rules="[rules.required, rules.length]" maxlength="6"
                    variant="outlined" class="mt-4"></v-text-field>
                <v-alert v-if="message" :type="messageType" variant="tonal" class="mt-3">
                    {{ message }}
                </v-alert>
            </v-card-text>
            <v-card-actions>
