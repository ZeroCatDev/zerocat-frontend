<template>
  <v-container>
    <v-card class="mb-4" v-for="comment in comments" :key="comment.id">
      <v-card-item>
        ...
        <template v-slot:append>
          <v-btn icon="mdi-reply" @click="$refs.commentComponent.openAddComment({pid: comment.id})"> </v-btn>
        </template>
        <v-card-title :to="'/user/' + comment.user_id">{{ comment.nick }}</v-card-title>

        <v-card-subtitle>{{ comment.time }} - {{ comment.browser }} ({{ comment.os }})</v-card-subtitle>
      </v-card-item>
      <v-card-text>{{ comment.comment }}</v-card-text>
      <v-card class="pb-3 pl-10 pr-3" v-if="comment.children && comment.children.length">
        <v-list density="default" style="display: none;">
          <!-- Loop over only the first 3 child comments -->
          <v-list-item v-for="children in comment.children.slice(0, 3)" :key="children.id" link>

            <template v-slot:prepend>
              <v-img :src="comment.avatar"></v-img> </template>
    ...
    </v-card><v-btn @click="getComments" :disabled="loadbuttondisabled">继续加载</v-btn>
    ...
    <!-- 添加新评论 -->
<AddComment  url="/" :callback="() => getComments({ retry: true })" ref="commentComponent"/>
...
export default {
  ...
  methods: {
    getComments(info = {}) {
      ...
      // 获取评论列表
      request({
        url: `/comment/api/comment?url=/&page=${this.page + 1}&pageSize=${this.pageSize}&sortBy=insertedAt_desc`,
