        ...
        <v-card hover :loading="loading">
          ...
          <v-card-text class="bg-surface-light pt-4">
            未正确加载下载地址？您可以尝试在应用商店中下载 <a href="https://apps.apple.com/cn/app/scratch-desktop/id1446785996?mt=12"
              ...
              OS 版本</a> 和 <a href="https://www.microsoft.com/zh-cn/p/scratch-3/9pfgj25jl6x3" class="on-surface">Windows
              10
              以上版本</a> 的
            Scratch 桌面版。
...
export default {
  ...
  created() {
    ...
    request.get(scratch_proxy+'/asdm')
      ...
      .catch(err => {
        console.log(err)
        axios.get('https://api.github.com/repos/sunwuyuan/AutoScratchDesktopMirror/releases/latest')
          .then(res => {
            this.release.stat = 1
            this.release.url.windows = res.assets.
              filter((element) => { return (element['name'] == "scratch-win.exe") })[0]
