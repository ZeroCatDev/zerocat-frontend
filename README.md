
# ZeroCat 零猫社区

[![FOSSA Status](https://app.fossa.com/api/projects/git%2Bgithub.com%2FZeroCatDev%2Fzerocat-frontend.svg?type=shield)](https://app.fossa.com/projects/git%2Bgithub.com%2FZeroCatDev%2Fzerocat-frontend?ref=badge_shield)

[![FOSSA Status](https://app.fossa.com/api/projects/git%2Bgithub.com%2FZeroCatDev%2Fzerocat-frontend.svg?type=large)](https://app.fossa.com/projects/git%2Bgithub.com%2FZeroCatDev%2Fzerocat-frontend?ref=badge_large)


新一代，开源，编程社区

访问 [ZeroCat 零猫社区](https://zerocat.dev)

查看主要仓库 [ZeroCatDev/ZeroCat](https://github.com/ZeroCatDev/ZeroCat)

加入我们的 [QQ群 964979747](https://qm.qq.com/q/hc8b3f5u38)

## 环境变量

可以通过设置以下环境变量来配置项目：

- `VITE_SENTRY_DSN`: Sentry DSN (可选，默认值已配置)


## 开发者

[@SunWuyuan](https://github.com/sunwuyuan)

## 如何贡献

ZeroCat 非常欢迎你的加入！[提一个 Issue](https://github.com/ZeroCatDev/ZeroCat/issues/new) 或者提交一个 Pull Request。对于小白问题，最好在 qq 群里问，更加方便。

ZeroCat 的项目 遵循 [Contributor Covenant](http://contributor-covenant.org/version/1/3/0/) 行为规范
<br/>孙悟元 希望你遵循 [提问的智慧](https://github.com/ryanhanwu/How-To-Ask-Questions-The-Smart-Way/blob/main/README-zh_CN.md)

[![Contributors](https://contrib.rocks/image?repo=ZeroCatDev/zerocat-frontend)](https://github.com/ZeroCatDev/zerocat-frontend/graphs/contributors)


## License

ZeroCat 社区项目采用 [GNU Affero General Public License v3.0](LICENSE) 开源。

Copyright © 2020–2026 Sunwuyuan.<br/>
All rights reserved.